...
const TablePage = () => {
  ...
  const handleHover = useCallback((el_number) => {
      if (!X) return;
  ...
  return (
    <div className='body'>
      <div className='body-inner'>
        <label className='data-input'>M <input type="number" value={M} onChange={e => setM(e.target.value)}/></label>
        <label className='data-input'>N <input type="number" value={N} onChange={e => setN(e.target.value)}/></label>
        <label className='data-input'>X <input type="number" value={X} onChange={e => setX(e.target.value)}/></label>
        <Button onClick={addRow} className="my-4">Add row</Button>
      </div>
      {
        matrix.length > 0 &&
        <Table striped bordered hover>
          <thead>
          <tr>
            {matrix[0].map((el, index) => <th key={index}><b>{index + 1}</b></th>)}
          ...
          <tr>
            {
              matrix[0].map((lastRow, index) => <td key={index * totalInCol[index]}><b>{totalInCol[index]}</b></td>)
            }
            <td key='last'>-</td>
          </tr>
          </tbody>
